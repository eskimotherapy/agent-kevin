...
First regenerate the Agent OS dashboard so `<HOME>/index.html` snapshots the post-scan state (placement matters: step 6's task_scan auto-mutates task state, and the dashboard should reflect it):

```
mcp__plugin_agent-kevin_kevin__status_dashboard
```

Returns `{ path, bytes }`. One call, no judgment needed.

After all mutations above, pull the final task board and the lint report — these are your sources for the summary, not the per-tool return values:

```
mcp__plugin_agent-kevin_kevin__task_query with {}   # full board for the Tasks counts
...
- `morning` → run [morning-briefing](../morning-briefing/SKILL.md) verbatim. The Active Threads / task queries / scan results are already in context from step 7; skip the re-reads and go straight to the signal-news perplexity call, compose, **then call `report_write` per the briefing skill's `## Persist` section**. Compose-without-persist is the bug — the briefing isn't done until `reports/index.md` shows today's entry.
...
🖥 Dashboard — <HOME>/index.html refreshed
